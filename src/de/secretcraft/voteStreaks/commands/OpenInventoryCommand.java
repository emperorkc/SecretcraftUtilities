...
import de.secretcraft.voteStreaks.listeners.MenuListener.onClick;
...
public class OpenInventoryCommand implements CommandExecutor {
	...
	//if(votifierListener.isInList(l1,votedToday.getStreak(p))) {
			...
			MenuListener menu = new MenuListener("§3§lBelohnungen        §2Serie:§4§l "+votedToday.getStreak(p), 6);
			int j = 1;
			int k = 1;
		...
		while(l1.hasAccess()) {
			...
			if(!VotedPlayers.getRewardID(p, i).equals("LEGACY_SKULL_ITEM")) {
				...
				item=addRewardCommand.getItem(VotedPlayers.getRewardName(p, i), VotedPlayers.getRewardID(p, i), i, p, VotedPlayers.getRewardAmount(p, i));
				 ...
				 String uuid1="";
					...
					for(int i=1; i<=10; i++) {
						if(!l2.isEmpty()) {
							...
							if(u!=null) {
						p.sendMessage("         §2"+i+". §3"+u.getName()+": §4"+votedToday.getStreak(u)+ "§2 Tage");
							} else {
								if(u1.getName()!=null) {
								p.sendMessage("         §2"+i+". §3"+u1.getName()+": §4"+votedToday.getStreak(u1)+ "§2 Tage");
							}}
						l2.remove();
					}}
					...
					l3.toFirst();
						...
						if((Bukkit.getPlayer(playername)!=null) || tz==true ) {
							...
							if(h==null) {
								...
								p.sendMessage(prefix+"§3"+h1.getName()+" §2hat eine Vote-Serie von§4§l "+votedToday.getStreak(h1)+"§2!");
							} else {
								p.sendMessage(prefix+"§3"+h.getName()+" §2hat eine Vote-Serie von§4§l "+votedToday.getStreak(h)+"§2!");
							}
