...
public class MessengerCommand implements CommandExecutor {
	...
	public void start() {
		...
		Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
			public void run() {
				createMessengerBar(100, 0.15,
						"§4§k-----§r §2Wir suchen aktuell weitere Teammitglieder. §c-> /forum §4§k-----");
			}
		}, 100 * 3);
		Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
			public void run() {
				createMessengerBar(100, 0.2,
						"§4§k-----§r §2Zu den wichtigsten Orten gelangst du mit §6/teleport §4§k-----");
			}
		}, 400);
		Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
			public void run() {
		...
		Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
			public void run() {
				createMessengerBar(100, 0.8,
						"§4§k-----§r §2Die Votes werden alle §630 Minuten §2ausgewertet! §4§k-----");
			}
		}, 1600);
		Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
			public void run() {
				createMessengerBar(100, 0.85,
						"§4§k-----§r §2Für Information zu einem Grundstück: §6/gs info §4§k-----");
			}
		}, 1700);
		Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
			public void run() {
		...
		Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
			public void run() {
				createMessengerBar(100, 1,
						"§4§k-----§r §2Durch tägliches Voten erhöht sich deine Voteserie! §6/serie §4§k-----");
			}
		}, 2000);

		/*
		 * task= Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
		 * public void run() { List<String> l1= getPlayers(); l1.toFirst();
		 * Bukkit.getScheduler().runTaskTimer(plugin, new Runnable() {
		 * 
		 * public void run() { if(l1.hasAccess()) { createMessengerBar(200,
		 * 0.75+0.25/getAmount(),getMessage(l1.getObject()) ); l1.next(); } else {
		 * Bukkit.getScheduler().cancelTask(task);
		 * 
		 * }
		 * 
		 * try { save(); } catch (IOException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); }
		 * 
		 * }
		 * 
