...
public class BankCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender arg0, Command arg1, String arg2, String[] arg3) {
		...
		if (arg3.length > 0) {
					...
					if (Double.parseDouble(arg3[1]) != 0) {
						...
						if (eco.getBalance(p) >= Double.parseDouble(arg3[1])) {
							try {
								...
								p.sendMessage(prefix+" §e"+Double.parseDouble(arg3[1])+ " §6SD wurden auf dein Konto eingezahlt!");
					...
					if (Double.parseDouble(arg3[1]) != 0) {
						...
						if (BankData.getMoney(p) >= Double.parseDouble(arg3[1])) {
							try {
								...
								p.sendMessage(prefix+" §e"+Double.parseDouble(arg3[1])+ " §6SD wurden dir von deinem Konto ausgezahlt!");
							} catch (NumberFormatException e) {
							...
							p.sendMessage(prefix+ " §cDein Konto verfügt nicht über die angegebene Menge an SD!");
						}
					} else {						p.sendMessage(prefix+ " §cKorrekte Nutzung: /konto auszahlen <Betrag>!");
			...
			p.sendMessage(prefix+ " §cKorrekte Nutzung: /konto <einzahlen/auszahlen/kontostand/deaktivieren/aktivieren>");
		}} else {
			Output.Err(p, "perm");
		}
		return false;
