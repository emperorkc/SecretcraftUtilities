...
public class TokenCommands implements CommandExecutor {
					...
					if (arg3.length > 2) {
						...
						if (Bukkit.getOfflinePlayer(arg3[1]) != null) {
							...
							try {
								...
								PlayerTokenData.addToken(Bukkit.getOfflinePlayer(arg3[1]), Integer.parseInt(arg3[2]));
					...
					if (arg3.length > 2) {
						...
						if (Bukkit.getOfflinePlayer(arg3[1]) != null) {
							try {
								...
								PlayerTokenData.subToken(Bukkit.getOfflinePlayer(arg3[1]), Integer.parseInt(arg3[2]));
							} catch (NumberFormatException | IOException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							}
					...
					if (!l2.isEmpty()) {
						...
						if (u != null) {
							p.sendMessage("         §2" + i + ". §3" + u.getName() + ": §4"
									+ Math.round(PlayerTokenData.getToken(u)) + "§2 Token");
						} else {
							if (u1.getName() != null) {
								p.sendMessage("         §2" + i + ". §3" + u1.getName() + ": §4"
										+ Math.round(PlayerTokenData.getToken(u1)) + "§2 Token");
							}
						}
						l2.remove();
					}
	...
	public static void closeInv(Player p) {
		double token = Math.floor(PlayerTokenData.getToken(p) * 100);
		double tokeni = token / 100;
		MenuListenerToken menu = new MenuListenerToken("§3§lToken-Shop     §cToken:§a " + tokeni, 4);
	...
	public static void openInvEgg(Player p) {
		double token = Math.floor(PlayerTokenData.getToken(p) * 100);
		double tokeni = token / 100;
		MenuListenerToken menu = new MenuListenerToken("§3§lSpawneier     §cToken:§a " + tokeni, 3);
		...
		epm.setLore(Arrays.asList("§6Schlage hiermit eines der", "§6folgenden Tiere um eines", "§6einzufangen:",
				"§7Huhn", "§7Schaaf", "§7Schwein", "§7Kuh"));
	...
	public static void openInvHead(Player p) {
		double token = Math.floor(PlayerTokenData.getToken(p) * 100);
		double tokeni = token / 100;
		MenuListenerToken menu = new MenuListenerToken("§3§lDeko Seite 1  §cToken:§a " + tokeni, 6);
	...
	public static void openInvHead2(Player p) {
		double token = Math.floor(PlayerTokenData.getToken(p) * 100);
		double tokeni = token / 100;
		MenuListenerToken menu = new MenuListenerToken("§3§lDeko Seite 2  §cToken:§a " + tokeni, 6);
	...
	public static void openInvHead3(Player p) {
		double token = Math.floor(PlayerTokenData.getToken(p) * 100);
		double tokeni = token / 100;
		MenuListenerToken menu = new MenuListenerToken("§3§lDeko Seite 3  §cToken:§a " + tokeni, 6);
		...
		/*
		 * menu.addButton(menu.getRow(1), 7, cH("75ff6c5d-a38b-4155-b559-70c7f4d8a12f",
		 * "Herz"));
		 * 
		 * menu.addButton(menu.getRow(2), 1, cH("MHF_Instagram", "Instagram"));
		 * menu.addButton(menu.getRow(2), 2, cH("Baeumchen", "Fichtenbusch"));
	...
	public static void openInvSpec(Player p) {
		double token = Math.floor(PlayerTokenData.getToken(p) * 100);
		double tokeni = token / 100;
		MenuListenerToken menu = new MenuListenerToken("§3§lBesonderes" + "    §cToken:§a " + tokeni, 2);
	...
	public static void openInvPerm(Player p) {
		double token = Math.floor(PlayerTokenData.getToken(p) * 100);
		double tokeni = token / 100;
		MenuListenerToken menu = new MenuListenerToken("§3§lTemporäre-Rechte" + " §cToken:§a " + tokeni, 2);
